...
function tokenRuntimeState(token: WhitehashToken): ProjectState {
  const metadata =
    token.raw && typeof token.raw === "object" ? (token.raw as Record<string, unknown>) : {}
  const artifact = token.artifactUri ?? ""
  const inputBytes = /(?:#0x|[?&]fxparams=)([0-9a-f]+)/i.exec(artifact)?.[1]
  const definition = (Array.isArray(metadata.params) ? metadata.params : undefined) as
    | FxParamDefinitions
    | undefined
  ...
  return {
    ...
    snippetVersion:
      typeof metadata.snippetVersion === "string" ? metadata.snippetVersion : undefined,
    inputBytes,
    definition,
  }
}
...
export function Variations({ token }: { token: WhitehashToken }) {
  ...
  return (
    <div className="mt-5 grid gap-8 md:grid-cols-[1.4fr_1fr]">
      <ArtworkIframe
        ref={value.ref}
        title={`Variation of ${token.name ?? token.tokenId}`}
        className="aspect-square w-full rounded-lg border border-line bg-black"
      />
      <div>
        ...
        <p className="mt-2 text-sm leading-6 text-muted">
          Everything runs in your browser. Change the seed or declared fx(params); the controller
          rebuilds the content-addressed generator URL and reloads only this iframe.
        </p>
        <Field.Root className="mt-5">
          <Field.Label>Hash</Field.Label>
          <Field.Control
            render={
              <Input
                value={hash}
                onChange={event =>
                  value.controller.runtime().updateState({ hash: event.target.value })
                }
              />
            }
          />
        ...
        <Button
          className="mt-2"
          variant="secondary"
          disabled={!supportsSeedVariation}
          onClick={() => value.controller.runtime().updateState({ hash: freshHash(hash) })}
        >
          New hash
        </Button>
        {!supportsSeedVariation ? (
          <Callout className="mt-4">
            This token record does not include its project’s reusable <code>generatorUri</code>. Its
            minted <code>artifactUri</code> may have the original seed embedded, so it is not safe
            to use as a variation generator.
          </Callout>
        ) : null}
        {definitions.length ? (
          <div className="mt-6 space-y-4">
            {definitions.map(definition => (
              <Field.Root key={definition.id}>
                <Field.Label>{definition.name ?? definition.id}</Field.Label>
                <Field.Control
                  render={
                    <Input
                      defaultValue={String(
                        value.controls.params.values[definition.id] ??
                          definition.value ??
                          definition.default,
                      )}
                      onChange={event =>
                        value.controller
                          .controls()
                          .update(
                            { [definition.id]: coerceParam(event.target.value, definition) },
                            definitions,
                            { forceRefresh: true },
                          )
                      }
          ...
          <Callout className="mt-6">
            This token does not publish editable fx(params) definitions. Seed exploration is still
            available.
          </Callout>
        )}
        <p className="mt-5 break-all font-mono text-[11px] text-faint">
          {value.controller.getUrl()}
        </p>
      </div>
    </div>
  )
