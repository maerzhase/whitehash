...
export const UNDERSTAND_ENTRIES: UnderstandEntry[] = [
  { slug: "overview", title: "How Whitehash works", description: "The three ideas you need before using the guides and API reference." },
  { slug: "data-model", title: "Projects and tokens", description: "A project is the generator; a token is one minted iteration." },
...
function Overview() {
  return (
    <DocsPage>
      <DocsHeading eyebrow="Understand" title="How Whitehash works" description="Three ideas explain the API: projects generate tokens, previews are not live artwork, and reads go directly to public RPCs, indexers, and gateways." />
      <DocsSection title="1. A project generates tokens">
        <div className="docs-prose"><p>A <strong>project</strong> is a generative release. A <strong>token</strong> is one minted iteration with its own identity and deterministic seed. Whitehash exposes both directly instead of hiding them behind a generic collection object.</p></div>
        <CodeBlock className="mt-4" language="text" code={`project { chain, id }
  └─ token { chain, contract, tokenId }`} />
      </DocsSection>
      <DocsSection title="2. A preview is not the artwork">
        <div className="docs-prose"><p><code>displayUri</code> and <code>thumbnailUri</code> are static images. <code>artifactUri</code> is executable HTML. <code>Artwork</code> shows the preview first, then builds the correctly seeded live URL and runs it in a restricted iframe.</p></div>
      </DocsSection>
      <DocsSection title="3. One shape across every chain">
        <div className="docs-prose"><p>Tezos, Ethereum, and Base store and index fxhash work differently. Whitehash reads known contracts through configurable third-party services, normalizes the results into <code>WhitehashProject</code> and <code>WhitehashToken</code>, and resolves IPFS or onchfs content without a Whitehash-hosted backend.</p></div>
        <CodeBlock className="mt-4" language="text" code={`chain + public indexer/RPC
  → normalized project or token
  → content-addressed preview and live artwork`} />
      </DocsSection>
      <Callout>Want to verify the details? The <Link className="docs-text-link" href="/understand/sources">data sources</Link> and <Link className="docs-text-link" href="/understand/urls">URL anatomy</Link> pages document the complete path.</Callout>
    </DocsPage>
  )
}
...
function DataModel() {
  return (
    <DocsPage>
      <DocsHeading eyebrow="Understand" title="Projects and tokens" description="Two domain objects carry the API: a project is the generator; a token is one minted iteration." />
      <DocsSection title="Identity lives on the object">
        <div className="docs-prose">
          <p>Tokens identify themselves with <code>chain</code>, <code>contract</code>, and <code>tokenId</code>. Projects use <code>chain</code> and <code>id</code>. Pass those fields directly to read APIs.</p>
        </div>
        <CodeBlock className="mt-4" language="tsx" code={`const token = useToken({ chain, contract, tokenId })
      ...
      <DocsSection title="WhitehashProject — one generative project">
        <Table
          ...
          rows={[
            ...
            [<code>id</code>, <>EVM collection contract, or a Tezos issuer entry such as <code>v3:13623</code></>, "never"],
            ...
            [<code>captureSettings</code>, "Normalized project capture mode, trigger, resolution, delay, selector, GPU, and GIF settings", "project metadata does not publish capture configuration"],
          ]}
        />
      </DocsSection>
      <Callout><strong>Refs are optional.</strong> Use <code>projectRef()</code>, <code>tokenRef()</code>, and <code>formatRef()</code> only when a route or paste field needs one serialized value. Ordinary reads use the identity fields above.</Callout>
    </DocsPage>
  )
}
...
function Sources() {
  return (
    <DocsPage>
      <DocsHeading eyebrow="Understand" title="Where the data comes from" description="Every fact Whitehash shows comes from a chain, public indexer, RPC, or content-addressed file. Every trusted address is listed here and verifiable on a block explorer." />
...
function UrlAnatomy() {
  ...
  return (
    <DocsPage>
      <DocsHeading eyebrow="Understand" title="How URLs are built" description="A token's render URL carries the seed and parameters that make one iteration deterministic. Here is how Whitehash constructs and resolves it." />
...
└─ scheme  └─ generator CID   └─ seed        └─ iteration   └─ minter      └─ chain      └─ fx(params) bytes`} />
        <div className="docs-prose mt-4">
          ...
          <p><strong>seed (<code>fxhash</code>)</strong> — makes the iteration deterministic. For gentk-v1, the seed is <em>not</em> in the URI; Whitehash reads it from the separate <code>iterationHash</code> metadata field and appends it. v2/v3 and EVM embed it.</p>
...
function Glossary() {
  return (
    <DocsPage>
      <DocsHeading eyebrow="Understand" title="Glossary" description="Whitehash uses one term per concept, aligned with fxhash vocabulary across the API, components, and docs." />
      <DocsSection title="One language">
        <Table
          ...
          rows={[
            ...
            [<strong>ref</strong>, <>An optional serialized form of project or token identity, useful for routes and paste fields. Normal read APIs accept the object&rsquo;s identity fields directly.</>],
          ]}
        />
      </DocsSection>
