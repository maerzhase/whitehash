...
export function DocsHeading({ eyebrow, title, description }: { eyebrow?: string; title: ReactNode; description?: ReactNode }) {
  return (
    <header className="border-b border-line pb-10">
      {eyebrow ? <div className="mb-3 font-mono text-[11px] font-medium uppercase tracking-[0.12em] text-primary">{eyebrow}</div> : null}
      <h1 className="font-display text-4xl font-semibold leading-tight tracking-[-0.05em] sm:text-6xl">{title}</h1>
      {description ? <div className="mt-5 max-w-2xl text-base leading-relaxed text-muted sm:text-lg">{description}</div> : null}
    </header>
  )
}
...
export function LiveDemo({ className, ...props }: ComponentProps<"div">) {
  return <div className={cx("overflow-hidden rounded-lg border border-line bg-canvas p-5", className)} {...props} />
}

export function Callout({ className, ...props }: ComponentProps<"aside">) {
  return <aside className={cx("border-l-2 border-primary bg-surface-2 px-4 py-3 text-sm leading-relaxed text-muted", className)} {...props} />
}
    ...
    <div className={cx("code-window group", className)}>
      ...
      <Highlight theme={themes.vsDark} code={code.trim()} language={language as Language}>
        {({ className: prismClass, style, tokens, getLineProps, getTokenProps }) => (
          <pre className={cx(prismClass, "overflow-x-auto p-4 font-mono text-[13px] leading-6")} style={{ ...style, background: "transparent" }}>
            {tokens.map((line, index) => <div key={index} {...getLineProps({ line })}>{line.map((token, tokenIndex) => <span key={tokenIndex} {...getTokenProps({ token })} />)}</div>)}
          </pre>
        )}
      </Highlight>
    </div>
