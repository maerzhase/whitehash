...
export function TokenGrid({
  ...
  return (
    <div className={cn("mt-8 grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-6", className)} {...props}>
      {children}
    </div>
  )
}
...
export function TokenGridSkeleton({ count = 8, className }: { count?: number; className?: string }) {
  return (
    <div className={cn("mt-8 grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-6", className)} aria-hidden>
      {Array.from({ length: count }, (_, index) => (
        <Card.Root key={index} className="shadow-[0_1px_2px_rgba(0,0,0,.16)]">
          ...
          <Card.Body><Skeleton className="h-4 w-2/3" /><Skeleton className="h-4 w-16" /></Card.Body>
        </Card.Root>
      ))}
    </div>
  )
